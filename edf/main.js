...
var $, burstDamage, critAvg, curvedMagDamage, falloff, growingMagDamage, shotDamage;
...
window.magDamage = (wpn) => {
  ...
  if (wpn.ammoDamageCurve || wpn.ammoCountCurve) {
    ...
    return shotDamage(wpn) * Math.ceil((wpn.ammo || 1) / (wpn.drain || 1));
  }
};
...
window.selectDropdown = (scope) => {
  var el, len2, n, ref;
  ref = document.querySelectorAll('.dropdown');
  for (n = 0, len2 = ref.length; n < len2; n++) {
    el = ref[n];
    el.classList.remove('hover');
  }
  if (locals.openMenu === scope) {
    return locals.openMenu = null;
  } else {
    locals.openMenu = scope;
    return document.getElementById(`dropdown-${scope}`).classList.add('hover');
  }
};
...
loadData = async(gameId) => {
  ...
  render();
  data = (await fetch(`./edf/weapons-${locals.game.num}.json`).then((res) => {
    return res.json();
  }));
...
var processHeaders = () => {
  ...
  console.log(_headers__WEBPACK_IMPORTED_MODULE_0__.headers);
...
var block = (this && this.block), attributes = (this && this.attributes) || {};
pug_html = pug_html + "\u003Cdiv" + (pug.attr("class", pug.classes(["dropdown",{ widechar: gameValues && gameValues.widechar }], [false,true]), false, true)+pug.attr("id", `dropdown-${scope}`, true, true)+pug.attr("onclick", `selectDropdown('${scope}')`, true, true)) + "\u003E\u003Cdiv" + (pug.attr("class", pug.classes(["button",{ [selection.class || selection.id]: true }], [false,true]), false, true)) + "\u003E";
...
pug_html = pug_html + "\u003Cth" + (pug.attr("title", (h.tooltip || ''), true, true)+pug.attr("colspan", (h.colspan || 1), true, true)) + "\u003E" + (pug.escape(null == (pug_interp = h.label) ? "" : pug_interp)) + "\u003C\u002Fth\u003E";
      }
  } else {
    var $$l = 0;
